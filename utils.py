...
def build_finetune_model(model_name, model_new, model_pretrained, num_rnn_layers,
                         num_layers_frozen=0):
    """
    ...
    elif model_name == "gglrnn":
        for l in range(num_rnn_layers):
            model_new.encoder.encoding_cells[l].dconv_gate = model_pretrained.encoder.encoding_cells[l].ggnn_gate
            model_new.encoder.encoding_cells[l].dconv_candidate = model_pretrained.encoder.encoding_cells[l].ggnn_candidate
    else:
        raise ValueError(
            'For fine-tuning, provide pretrained model error!')
