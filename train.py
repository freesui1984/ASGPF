...
from data.dataloader_bids_classification import load_bids_dataset_classification
from data.dataloader_densecnn_classification import load_dataset_densecnn_classification
from data.dataloader_bids_densecnn_classification import load_bids_densecnn_classification
from constants import *
from args import get_args
from collections import OrderedDict
from json import dumps
from model.model import DCRNNModel_classification, DCRNNModel_nextTimePred
from model.gnn import GGNN, GCN
from model.ASGPFmodel import SGLCModel_classification, SGLCModel_nextTimePred
from model.eeg_graph_clf import EEGGraphClf, FocalLoss
from model.Transformer import TransformerModel_nextTimePred, TransformerModel_classification
...
def main(args):
    ...
    if args.task == 'detection':
        ...
        if args.model_name != 'densecnn' and args.dataset_name == 'tusz':
    ...
    if args.do_train:
        if not args.fine_tune:
            ...
            if args.load_model_path is not None:
                ...
                if args.model_name == "dcrnn":
                    ...
                    pretrained_model = SGLCModel_nextTimePred(
                        args=args_pretrained, device=device)  # placeholder
                else:
                    raise NotImplementedError
        ...
        # Train
        train_loss_list, eval_loss_list = train(model, dataloaders, args, device, args.save_dir, log, tbx)
        train_loss_array = np.array(train_loss_list)
        eval_loss_array = np.array(eval_loss_list)
        train_loss_save_path = os.path.join(args.save_dir, 'train_loss_list.npy')
        eval_loss_save_path = os.path.join(args.save_dir, 'eval_loss_list.npy')
        np.save(train_loss_save_path, train_loss_array)
        np.save(eval_loss_save_path, eval_loss_array)
    ...
    if args.num_classes == 4:
        dev_results, dev_str_auroc = evaluate(model,
                               dataloaders['dev'],
                               args,
                               args.save_dir,
                               device,
    ...
    dev_results_str = ', '.join('{}: {:.5f}'.format(k, v)
                                for k, v in dev_results.items())
    log.info('DEV set prediction results: {}'.format(dev_results_str))
    if args.num_classes == 4:
        log.info('DEV set AUROC results: {}'.format(dev_str_auroc))

    if args.num_classes == 4:
        test_results, test_str_auroc = evaluate(model,
                                dataloaders['test'],
                                args,
                                args.save_dir,
                                device,
    ...
    # Log to console
    test_results_str = ', '.join('{}: {:.5f}'.format(k, v)
                                 for k, v in test_results.items())
    log.info('TEST set prediction results: {}'.format(test_results_str))
    if args.num_classes == 4:
        log.info('TEST set AUROC results: {}'.format(test_str_auroc))
...
def train(model, dataloaders, args, device, save_dir, log, tbx):
    ...
    eval_loss_list = []
    train_loss_list = []
    if args.use_graph_learning:
        print('[ Graph Learner ]')
        learngraph = EEGGraphClf(args, device)
        ...
        with torch.enable_grad(), \
            ...
            for x, y, seq_lengths, supports, _, _ in train_loader:
                ...
                if args.model_name == "dcrnn":
                    ...
                    adjacency_matrix = torch.stack(supports)
                    init_adj = torch.squeeze(adjacency_matrix, dim=0)
                    logits = model(x, seq_lengths, init_adj)
                    if logits.shape[-1] == 1:
                        logits = logits.view(-1)  # (batch_size,)
                    # y = y.float()
                    loss = loss_fn(logits, y)
                ...
                tbx.add_scalar('train/LR',
                               ...
                               step)
            train_loss = nll_meter.avg if (nll_meter is not None) else loss.item()
            log.info('train_loss {:.5f}'.format(train_loss))

            if epoch % args.eval_every == 0:
                # Evaluate and save checkpoint
                log.info('Evaluating at epoch {}...'.format(epoch))
                if args.num_classes == 4:
                    eval_results, dev_str_auroc = evaluate(model,
                                            dev_loader,
                                            args,
                                            save_dir,
                                            device,
                ...
                eval_loss_list.append(eval_results['loss'])
                ...
                # Log to console
                results_str = ', '.join('{}: {:.5f}'.format(k, v)
...
def evaluate(
    ...
    if args.num_classes == 4:
        auroc_dict = {}
        for class_label in range(3):  # 请替换为实际的类别数量
            y_true_class = (y_true_all == class_label).astype(int)
            y_prob_class = y_prob_all[:, class_label]
    ...
    if (args.num_classes == 1) and (eval_set == 'dev') and is_test:
    ...
    scores_dict, _, _ = utils.eval_dict(y_pred=y_pred_all,
                                        ...
                                        average="binary" if args.num_classes == 1 else "weighted")
    ...
    if args.num_classes == 4:
        return results, format_str_auroc
    else:
        return results
