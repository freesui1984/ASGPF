...
def compute_PSD_features(signals, sampling_freq):
    """
    Args:
    ...
    for channel_data in signals:
        freqs, psd = welch(channel_data, fs=sampling_freq, nperseg=200, nfft=200)

        # 筛选 fmin 到 fmax 之间的 PSD 值
        valid_indices = (freqs >= 0.5) & (freqs <= 100)
        psd_values.extend(psd[valid_indices])
    psd_feature = np.array(psd_values).reshape(len(signals), -1)
    return psd_feature
...
def get_swap_pairs(channels):
    ...
    if ("Fp1" in channels) and ("Fp2" in channels):
    ...
    if ("F7" in channels) and ("F8" in channels):
        swap_pairs.append((channels.index("F7"), channels.index("F8")))
    if ("C3" in channels) and ("C4" in channels):
        swap_pairs.append((channels.index("C3"), channels.index("C4")))
    if ("T3" in channels) and ("T4" in channels):
        swap_pairs.append((channels.index("T3"), channels.index("T4")))
    if ("T5" in channels) and ("T6" in channels):
        swap_pairs.append((channels.index("T5"), channels.index("T6")))
    if ("O1" in channels) and ("O2" in channels):
...
def get_BIDS_OrderedChannels(file_name, verbose, labels_object, channel_names):
    labels = list(labels_object)
...
def getAlzheimerTimes(file_name, clip_len, alzheimer_idx):
    """
    Args:
        file_name: edf file name
    Returns:
    ...
    """
    raw_data = mne.io.read_raw_eeglab(file_name, verbose=False, preload=True)
    alzheimer_times = []

    return alzheimer_times
...
def getSETsignals(set_filepath):
    """
    ...
    try:
        ...
        # 提取数据和采样频率
        signals = raw.get_data()  # shape will be (num_channels, num_data_points)
        sample_freq = raw.info['sfreq']

        return signals, sample_freq
    except Exception as e:
        print(f"Error reading {set_filepath}: {e}")
        return None, None
