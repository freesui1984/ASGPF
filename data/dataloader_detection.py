import sys
sys.path.append('E:\\hy\\eeg-gnn-ssl-main')
import pyedflib
import utils
...
repo_paths = str(Path.cwd()).split('ASGPF')
repo_paths = Path(repo_paths[0]).joinpath('ASGPF')
sys.path.append(repo_paths)
FILEMARKER_DIR = Path(repo_paths).joinpath('data/tusz_detection')
# FILEMARKER_DIR = Path(repo_paths).joinpath('data/tusz_detection_subset')
